...
const handler: TaskHandler = {
  ...
  buildPrompt(input: PromptBuildInput) {
    ...
    if (mode === 'volume') {
      return {
        system: `${capabilityPreamble.system}\n\n你是小说分卷规划增强助手。用户正在编辑一个分卷信息，请根据已有内容和项目上下文对每个字段进行补充或优化。对于空字段，必须基于已有分卷、节点、角色和世界观锚点直接生成合理内容，不要向用户提问。对于已有内容的字段，在保留作者意图的基础上扩展和完善。请只返回 JSON 对象，不要返回 Markdown。必须同时返回 title、wordTarget、summary 三个字段。${formatStoryStateConstraint(context)}`,
    ...
    return {
      system: `${capabilityPreamble.system}\n\n你是小说剧情大纲增强助手。用户正在编辑一个大纲节点，请根据已有内容和项目上下文对每个字段进行补充或优化。对于空字段，必须基于当前分卷、前后节点、角色和世界观锚点直接生成合理内容，不要向用户提问。对于已有内容的字段，在保留作者意图的基础上扩展和完善。请只返回 JSON 对象，不要返回 Markdown；大纲模式必须返回 relatedCharacterIds、relatedOrganizationIds、relatedWorldviewIds，且只能使用上下文已有实体 id；角色参考非空时至少填写一个直接相关角色的 id。${formatStoryStateConstraint(context)}`,
