...
import {
  ...
  formatCurrentOutlineItem, formatInspirationEntries, formatOutlineItems, formatRelatedChapters,
...
【自检规则】
当你输出超过 500 字的创作内容（续写、重写、整段生成）后，主动执行以下检查，不需要等用户要求：
- 字数是否达到用户期望（如有明确要求）
- 与前文是否衔接（最后一句是否能接上下文）
- 是否出现了上面列出的禁止失败模式
- 是否存在高疲劳词过度使用
如发现问题，在正文输出后简要说明并给出修正版本，格式："⚠ 自检发现：[问题]，已修正如下：[修正片段]"。无问题则不需要额外说明。`
