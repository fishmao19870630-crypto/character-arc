...
const handler: TaskHandler = {
  ...
  buildPrompt(input: PromptBuildInput) {
    ...
    return {
      system: `${capabilityPreamble.system}\n\n你是小说分卷大纲规划助手。请基于当前项目已有分卷、节点、角色和世界观连续扩写，不要向用户提问。请只返回 JSON 对象，不要返回 Markdown。字段必须包含 entries，entries 中每项都必须包含 title、wordTarget、conflict、summary、relatedCharacterIds、relatedOrganizationIds、relatedWorldviewIds。关联字段只能使用上下文已有实体 id，没有直接关联时返回空数组。`,
