...
const handler: TaskHandler = {
  ...
  buildPrompt(input: PromptBuildInput) {
    ...
    return {
      system: `${capabilityPreamble.system}\n\n你是小说世界观设定增强助手。用户正在编辑一条世界观词条，请根据已有内容和项目上下文对每个字段进行补充或优化。对于空字段，必须根据项目锚点直接生成，不要向用户提问。对于已有内容的字段，在保留作者意图的基础上扩展和完善。请只返回 JSON 对象，不要返回 Markdown。${formatStoryStateConstraint(context)}`,
