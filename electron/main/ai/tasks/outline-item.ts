...
const handler: TaskHandler = {
  ...
  buildPrompt(input: PromptBuildInput) {
    ...
    return {
      system: `${capabilityPreamble.system}\n\n你是小说剧情大纲助手。请只返回 JSON 对象，不要返回 Markdown。字段必须包含 title、wordTarget、conflict、summary、relatedCharacterIds、relatedOrganizationIds、relatedWorldviewIds。关联字段只能填写上下文中已有实体的 id；角色参考非空时，relatedCharacterIds 至少填写一个直接相关角色的 id。`,
